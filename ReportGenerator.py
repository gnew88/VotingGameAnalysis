...
class reportGenerator:
    ...
    def data_analysis(self, pq, no_pq, paired_table, largePq_int_all, largePq_int_f, largePq_int_sin, largePq_no_all, largePq_no_sin,
        ...
        tr_sb_p.add_run('\n- 實驗組的 wtp 為原始數據，至於控制組則是以五種方法計算，包含兩個遊戲之總和、取最大值、取最小值、取有門檻遊戲、沒門檻遊戲')
        ...
        self.document.add_heading('控制組的 wtp 為有門檻遊戲 wtp ', level = 2)
        self.document.add_heading('線性迴歸模型結果', level = 3)
        self.add_table(pq_reg)
        self.document.add_heading('F 檢定', level = 3)
        self.add_table(pq_f)

        self.document.add_heading('控制組的 wtp 為沒門檻遊戲 wtp', level = 2)
        self.document.add_heading('線性迴歸模型結果', level = 3)
        self.add_table(no_pq_reg)
        self.document.add_heading('F 檢定', level = 3)
        self.add_table(no_pq_f)
