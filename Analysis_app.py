...
from DownloadData import download_link
...
try:
    ...
    all_group_result = pd.DataFrame(all_group_result).reset_index(drop = True)
    all_group_result['wtp 平均比較'] = all_group_result.apply(lambda x : x['控制組 wtp 平均'] > x['實驗組 wtp 平均'], axis = 1)
    all_group_result.loc[(all_group_result['wtp 平均比較'] == 1), 'wtp 平均比較'] = '控制組 > 實驗組'
    all_group_result.loc[(all_group_result['wtp 平均比較'] == 0), 'wtp 平均比較'] = '實驗組 > 控制組' 

    all_group_result = all_group_result.drop(['t-statistic'], axis = 1)
    all_group_result = all_group_result[['沒門檻/有門檻', '控制組 wtp 平均', '實驗組 wtp 平均', 'wtp 平均比較', 'p-value']]
